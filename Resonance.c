/*
 * ════════════════════════════════════════════════════════════════
 *   RESONANCE: BLOOD DEBT  —  Simulador Completo v0.9.0
 *   Lenguaje: C
 * ════════════════════════════════════════════════════════════════
 *
 *  CONCEPTOS APLICADOS:
 *  A. Informatica   : tipos de variables, structs, punteros,
 *                     arreglos, ciclos, condicionales
 *  B. Matematica    : polinomios, valor absoluto, inecuaciones
 *  C. Logica : conectivos &&/||, cuantificadores,
 *                     teoria de conjuntos, teoria de grafos,
 *                     inferencia logica
 *
 *  FACCIONES:
 *   0 VIGILANTES  -> PROTOCOLO ESCUDO  (+25% DEF, ventaja x1.2)
 *   1 DISONANTES  -> PULSO CAOTICO     (+20% ATQ, 30% paralisis)
 *   2 SINDICATO   -> PROTOCOLO NULO    (+15% ATQ/HAB, penetra 20% DEF)
 *   3 ARQUITECTOS -> ECO ANCESTRAL     (+20% HP, legado 15% al morir)
 *
 *  CICLO DE VENTAJA (x1.5) — GRAFO DIRIGIDO CICLICO:
 *   VIGILANTES -> DISONANTES -> SINDICATO -> ARQUITECTOS -> VIGILANTES
 *
 *  ESTRUCTURA DEL ARCHIVO:
 *   1. Includes y defines
 *   2. Enumeraciones y estructuras
 *   3. Datos globales (catalogo, habilidades, cartas)
 *   4. Prototipos de funciones
 *   5. MAIN (al principio como lo pidio el profesor)
 *   6. Implementacion de todas las funciones
 * ════════════════════════════════════════════════════════════════
 */
...
/* Delay de batalla automatica en milisegundos */
#define BATTLE_DELAY_SHORT  800
#define BATTLE_DELAY_MEDIUM 1400
#define BATTLE_DELAY_LONG   2000
...
/* Control de idioma global: 0 = Espanol, 1 = English */
...
/* Habilidad de ataque basico (sin costo de energia) */
...
void        print_double_divider(void);
...
void display_both_fighters(Champion *attacker, Champion *defender);
...
/* Motor de calculo — CALCULO DE DEFENSA */
float     calculate_defense(Champion *attacker, Champion *defender);
...
int main(void)
{
    ...
    do
    {
        ...
        switch (menu_option)
        {
            ...
            case 2:
                printf(BOLD "\n  %s\n" RST,
                       (current_language==0)?"MODO DE JUEGO":"GAME MODE");
                printf("  1. PvP -- %s\n",
                ...
                {
                    int m;
                    if (scanf("%d", &m) != 1) { clear_input_buffer(); break; }
                    clear_input_buffer();
            ...
            case 3:
                ...
                if (current_mode == MODE_PVP)
                    create_team_manual(&team_2, 2);
                else
                    create_team_cpu(&team_2);
                teams_ready      = 1;
                combat_completed = 0;
                press_to_continue();
                break;
            ...
            default:
                printf(RED "  x %s\n" RST,
                       (current_language==0)?"Opcion invalida.":"Invalid option.");
                press_to_continue();
        }
    }
    while (menu_option != 7);
...
void display_skills(Champion *c)
{
    ...
    if (current_language == 0)
        printf(BOLD "  Habilidades -- %s%s%s (EN:%d/%d)\n" RST,
               color, get_name(c), RST, c->energy_current, c->energy_max);
    else
        printf(BOLD "  Skills -- %s%s%s (EN:%d/%d)\n" RST,
               color, get_name(c), RST, c->energy_current, c->energy_max);
...
void display_faction_cards(Faction f)
{
    ...
    if (current_language == 0)
        printf(BOLD "  Cartas de Mejora -- %s%s%s\n" RST,
               get_faction_color(f), get_faction_name(f), RST);
    else
        printf(BOLD "  Buff Cards -- %s%s%s\n" RST,
               get_faction_color(f), get_faction_name(f), RST);
...
/* ------------------------------------------------------------
 *  Muestra estado de un luchador (barra HP + barra escudo/DEF)
 * ------------------------------------------------------------ */
void display_fighter_state(Champion *c, const char *label)
{
    const char *color = get_faction_color(c->faction);
    ...
    printf("    SHIELD "); print_hp_bar((int)c->defense, 100, YLW);         printf("  DEF:%.1f\n", c->defense);
    printf("    EN     "); print_energy_bar(c->energy_current, c->energy_max, color); printf("\n");
    printf("    " DIM "ATK:%-6.1f SKL:%-6.1f" RST "\n", c->attack, c->skill);
}

/* Muestra ambos luchadores lado a lado en pantalla */
void display_both_fighters(Champion *attacker, Champion *defender)
{
    print_double_divider();
    printf("\n");
    display_fighter_state(attacker,
        (current_language==0) ? ">> ACTIVO" : ">> ACTIVE");
    printf("\n");
    display_fighter_state(defender,
        (current_language==0) ? "<< ENEMIGO" : "<< ENEMY");
    printf("\n");
    print_divider();
}
...
void create_team_manual(Team *t, int player_num)
{
    ...
    while (count < 5)
    {
        ...
        {
            printf(RED "  x %s\n" RST,
                   (current_language==0)?"ID invalido (1-12).":"Invalid ID (1-12).");
            continue;
        }
        if (check_already_chosen(chosen, count, id))
        {
            printf(RED "  x %s\n" RST,
                   (current_language==0)?"Ya elegido.":"Already selected.");
            continue;
        }
        chosen[count] = id;
        t->members[count] = champions_catalogue[id-1];
        for (int s = 0; s < 3; s++)
            t->members[count].skills[s] = skills_table[id-1][s];
        printf(GRN "  + %s%s%s (%s) -- %s\n" RST,
               get_faction_color(t->members[count].faction),
               get_name(&t->members[count]), RST,
               get_faction_name(t->members[count].faction),
               get_role_name(t->members[count].role));
        count++;
    }
    t->count            = 5;
    t->total_team_damage = 0;
    t->casualties        = 0;
}
...
void create_team_cpu(Team *t)
{
    ...
    t->casualties        = 0;
    printf(YLW "\n  [CPU] %s \"%s\"\n" RST,
           (current_language==0)?"Equipo generado:":"Random team:", t->name);
}
...
/* ------------------------------------------------------------
 *  CALCULO DE DEFENSA — funcion modularizada
 *
 *  Formulas aplicadas (Matematica B):
 *    effective_def = defender->defense
 *                    (si SYNDICATE con buff: * 0.80 — penetracion)
 *    mitigation    = effective_def - (attacker->skill * 0.1)
 *    inecuacion    : mitigation >= 0 siempre (valor absoluto minimo)
 *
 *  Logica proposicional (C):
 *    Conectivo && : SYNDICATE && buff_active activa penetracion
 * ------------------------------------------------------------ */
float calculate_defense(Champion *attacker, Champion *defender)
{
    /* Paso 1 — DEF efectiva con posible penetracion del Sindicato */
    float effective_def = (attacker->faction == SYNDICATE && attacker->buff_active)
                          ? (defender->defense * 0.80f)
                          : defender->defense;

    /* Paso 2 — La habilidad del atacante erosiona la defensa (lineal) */
    float mitigation = effective_def - (attacker->skill * 0.1f);

    /* Paso 3 — Inecuacion: mitigation nunca puede ser negativa */
...
/*
 * verify_advantage — ciclo de dominancia entre facciones
 * Grafo dirigido ciclico (Teoria de Grafos — Tema 5):
 *   WARDENS -> DISSONANTS -> SYNDICATE -> ARCHITECTS -> WARDENS
 */
int verify_advantage(Faction attacker, Faction defender)
{
...
/* Cuantificador existencial: existe algun miembro vivo? */
int has_alive_members(Team *t)
{
    for (int i = 0; i < t->count; i++)
        if (t->members[i].is_alive) return 1;
    return 0;
}
...
{
    for (int i = 0; i < t->count; i++)
        if (t->members[i].is_alive) return &t->members[i];
    return NULL;
}

/*
 * execute_attack — nucleo del combate
 *
 * Formula de dano (Matematica B — polinomio grado 2):
 *   a          = attack / 10
 *   base_power = 2a^2 + 3a + 5
 *   damage     = base_power * skill_multiplier * faction_multiplier
 *   hp_impact  = damage - mitigation (calculate_defense)
 */
void execute_attack(Champion *attacker, Champion *defender, Team *def_team, Skill *s)
{
    /* Paso 1 — Potencia base: polinomio cuadratico */
    float a          = attacker->attack / 10.0f;
    float base_power = (2.0f * a * a) + (3.0f * a) + 5.0f;
    float damage     = base_power * s->damage_multiplier;

    /* Paso 2 — Caso especial Ejecucion */
    ...
    /* Paso 5 — Mitigacion de defensa (funcion modularizada) */
    float mitigation = calculate_defense(attacker, defender);

    /* --- Mostrar encabezado del ataque --- */
    printf("\n  %s%-22s%s --> %s%-22s%s\n",
    ...
    if (s->healing_percentage != 0)
    {
        ...
        if (s->healing_percentage > 0)
        {
            ...
            if (attacker->hp_current > attacker->hp_max)
                attacker->hp_current = attacker->hp_max;
    ...
    {
        /* El escudo (DEF) absorbe todo el dano */
        defender->defense -= damage;
        if (defender->defense < 0.0f) defender->defense = 0.0f;
        printf("  " CYN "  [%s %.0f | SHIELD DEF %.1f]" RST "\n",
               (current_language==0)?"Escudo absorbe":"Shield absorbs",
               damage, defender->defense);
    }
    else
    {
        /* El dano supera el escudo y golpea los HP */
    ...
    /* Revertir buffs/nerfs temporales */
    if (!s->is_permanent)
    {
    ...
    /* Muerte y legado de Arquitectos */
    if (defender->hp_current <= 0)
    {
        ...
        printf("  " BOLD RED "  !! %s %s" RST "\n",
               get_name(defender),
               (current_language==0)?"derrotado.":"defeated.");
        if (defender->faction == ARCHITECTS && defender->buff_active)
        {
            ...
            if (ally != NULL)
            {
                ...
                if (ally->hp_current > ally->hp_max)
...
Skill *cpu_choose_skill(Champion *attacker)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (attacker->energy_current >= s->energy_cost &&
            s->damage_multiplier > highest)
        {
            highest = s->damage_multiplier;
            best    = s;
        }
    }
    if (best != NULL)
...
void menu_choose_card(Champion *attacker)
{
    ...
    printf("\n  %s (0-4): ",
           (current_language==0)?"Carta de mejora":"Buff card");
    int choice;
    while (1)
    {
...
/*
 * start_combat — bucle principal de batalla AUTOMATICA
 *
 * La batalla se ejecuta sola con delays entre cada accion
 * para que se puedan ver claramente las formulas y resultados.
 * El jugador elige sus habilidades; la CPU actua automaticamente.
 */
void start_combat(Team *team_a, Team *team_b, GameMode mode)
{
    int round_counter    = 1;
    int active_team_turn = 0;
    ...
    while (has_alive_members(team_a) && has_alive_members(team_b))
    {
        ...
        /* Mostrar ambos luchadores en pantalla */
        display_both_fighters(attacker, defender);
        sleep_ms(BATTLE_DELAY_SHORT);

        /* Resolver estados alterados */
        if (resolve_statuses(attacker))
        {
            attacker->energy_current++;
            if (attacker->energy_current > attacker->energy_max)
                attacker->energy_current = attacker->energy_max;
            active_team_turn = 1 - active_team_turn;
            round_counter++;
            continue;
        }
        ...
        if (is_cpu)
        {
            chosen_skill = cpu_choose_skill(attacker);
        }
        else
        {
            display_skills(attacker);
            printf("\n  %s (0-3): ",
                   (current_language==0)?"Elige habilidad":"Choose skill");
            int choice;
            while (1)
            {
                if (scanf("%d", &choice) != 1) { clear_input_buffer(); continue; }
            ...
            {
                chosen_skill = &attacker->skills[choice-1];
                if (attacker->energy_current < chosen_skill->energy_cost)
                {
                    printf(RED "  Energia insuficiente. %s.\n" RST,
                           (current_language==0)
                           ?"Ataque basico automatico":"Basic attack forced");
                    chosen_skill = NULL;
                }
                else
                {
                    attacker->energy_current -= chosen_skill->energy_cost;
                }
            }
        }
        ...
        /* Ejecutar ataque con delay automatico */
        sleep_ms(BATTLE_DELAY_SHORT);
        execute_attack(attacker, defender, def_team, chosen_skill);

        /* Nuevo luchador entra si el anterior cayo */
        if (!defender->is_alive)
        {
            ...
            if (backup != NULL)
            {
                printf("  %s  >> %s %s\n\n",
                       ...
                       get_name(backup),
                       (current_language==0)?"entra al campo.":"enters the field.");
                sleep_ms(BATTLE_DELAY_MEDIUM);
            }
        }
...
void display_final_statistics(Team *team_a, Team *team_b)
{
    ...
    if      (alive_a > 0)
        printf(BOLD GRN "  ** %s: %s\n\n" RST,
               (current_language==0)?"GANADOR":"WINNER", team_a->name);
    else if (alive_b > 0)
        printf(BOLD GRN "  ** %s: %s\n\n" RST,
               (current_language==0)?"GANADOR":"WINNER", team_b->name);
    else
        printf(BOLD YLW "  ** %s\n\n" RST,
               (current_language==0)?"EMPATE":"DRAW");
    ...
    for (int i = 0; i < 5; i++)
    {
        if (team_a->members[i].total_damage_dealt > mvp->total_damage_dealt)
            mvp = &team_a->members[i];
        if (team_b->members[i].total_damage_dealt > mvp->total_damage_dealt)
            mvp = &team_b->members[i];
    }
    printf(BOLD YLW "\n  >> MVP: %s%s%s -- %d DMG\n" RST,
           get_faction_color(mvp->faction), get_name(mvp), RST,
           mvp->total_damage_dealt);
    print_double_divider();
}
